...
export function analyzeProcurement(operation: string, lines: RfxLine[], vendors: AnalysisVendor[]) {
  ...
  if (operation === 'QUALIFIED_VENDORS') return { operation, qualified: qualified.map((vendor) => vendor.name), vendors: vendors.map((vendor) => ({ vendor: vendor.name, status: vendor.qualification })) };
  if (operation === 'CHEAPEST_OVERALL') return { operation, cheapest: cheapestSingle, vendorTotals, caveat: cheapestSingle ? null : 'No qualified vendor has an eligible quote for all 30 lines.' };
  if (operation === 'CHEAPEST_QUALIFIED_PER_LINE') return { operation, selections, unawardedLines: selections.filter((item) => !item).length, total: splitTotal };
  if (operation === 'UNRESOLVED_ITEMS') return { operation, unresolved, count: unresolved.length };
  if (operation === 'SPLIT_AWARD_SAVINGS') return { operation, splitTotal, cheapestSingle, savings: cheapestSingle ? cheapestSingle.total - splitTotal : null, selections, caveat: cheapestSingle ? null : 'Savings cannot be calculated because no qualified single vendor covers every eligible line.' };
  return { operation: 'SUMMARY', qualified: qualified.length, unresolved: unresolved.length, vendorTotals, splitTotal };
}
