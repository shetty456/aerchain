...
function RfxView({ event, expanded, setExpanded, aiConfigured, sent, sendRfx }: { event: SourcingEvent; expanded: string | null; setExpanded: (line: string | null) => void; aiConfigured: boolean; sent: boolean; sendRfx: () => void }) {
  return <>
    <section className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-[var(--line-strong)] bg-white px-5 py-4">
      <div><p className="text-sm font-semibold">{sent ? 'RFx sent to five vendors' : aiConfigured ? 'Ready to send' : 'AI keys required'}</p><p className="mt-1 text-xs leading-5 text-[var(--muted)]">{sent ? 'Open Responses to follow processing and review any failures.' : aiConfigured ? '30 line items and five invited vendors are ready for the simulated response journey.' : 'Add SARVAM_API_KEY and GROQ_API_KEY to .env.local, then restart the development server.'}</p></div>
