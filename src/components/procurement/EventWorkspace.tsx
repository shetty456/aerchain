  ...
  return <div className="min-h-screen bg-[var(--surface)]">
    <header className="sticky top-0 z-30 border-b border-[var(--line)] bg-white/95 backdrop-blur">
      <div className="flex h-14 items-center justify-between px-5">
        ...
        <div className="flex items-center gap-3"><span className={`status-pill ${aiConfigured ? 'status-ready' : 'status-error'}`}>{aiConfigured ? 'AI providers connected' : 'AI keys required'}</span>{sent && !running ? <button onClick={replayShowcase} className="flex items-center gap-2 rounded-lg border border-[var(--line-strong)] bg-white px-3.5 py-2 text-xs font-semibold hover:bg-[var(--surface)]"><RefreshCw size={13} /> Replay showcase</button> : <button disabled={!aiConfigured || sent || running} onClick={sendRfx} className="flex items-center gap-2 rounded-lg bg-[var(--ink)] px-3.5 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-55">{running ? <LoaderCircle className="animate-spin" size={13} /> : <Send size={13} />}{running ? `Processing ${Math.min(completedCount + errorCount + 1, 5)} of 5` : 'Send RFx'}</button>}</div>
      </div>
