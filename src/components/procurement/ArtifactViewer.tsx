...
export default function ArtifactViewer({ target, onClose }: { target: ArtifactViewerTarget; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/demo/artifacts/${target.vendorId}`, { cache: 'no-store' })
  ...
  return <div className="fixed inset-0 z-[70] flex flex-col bg-[var(--surface)]" role="dialog" aria-modal="true" aria-label={`${target.vendorName} original response`}>
    ...
    <main className="min-h-0 flex-1 overflow-auto">
      ...
      {view === 'DETAILS' && preview ? (preview.details ? <ExtractedDetails details={preview.details} /> : <div className="mx-auto mt-12 max-w-lg rounded-xl border border-[var(--line)] bg-white p-6 text-center"><ShieldCheck className="mx-auto text-[var(--muted)]" size={20} /><h2 className="mt-3 text-sm font-semibold">Available after processing</h2><p className="mt-2 text-xs leading-5 text-[var(--muted)]">Terms and compliance will appear here when this supplier response has finished processing.</p></div>) : <>
        ...
        {preview?.kind === 'XLSX' && <div className="flex min-h-full flex-col"><div className="flex items-center gap-3 border-b border-[var(--line)] bg-white px-3 py-3 sm:px-5"><label htmlFor="workbook-sheet" className="text-[10px] font-semibold uppercase tracking-wide text-[var(--muted)]">Workbook sheet</label><select id="workbook-sheet" value={sheet} onChange={(event) => setSheet(Number(event.target.value))} className="rounded-lg border border-[var(--line-strong)] bg-white px-3 py-2 text-[11px] font-semibold outline-none">{preview.sheets.map((item, index) => <option key={item.name} value={index}>{item.name}</option>)}</select></div><div className="flex-1 overflow-auto p-3 sm:p-5"><table className="min-w-full border-collapse bg-white text-left text-[11px] shadow-sm"><tbody>{activeSheet?.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-[var(--line)]">{row.map((cell, cellIndex) => <td key={cellIndex} className={`max-w-[360px] border-r border-[var(--line)] px-3 py-2 align-top ${rowIndex === 0 ? 'bg-[var(--surface)] font-semibold' : ''}`}>{cell}</td>)}</tr>)}</tbody></table></div></div>}
      </>}
    </main>
  </div>;
}
